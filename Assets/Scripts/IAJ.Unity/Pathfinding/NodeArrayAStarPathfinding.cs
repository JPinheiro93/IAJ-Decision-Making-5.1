    ...
    public class NodeArrayAStarPathfinding : AStarPathfinding
    {
        ...
        protected override void ProcessChildNode(NodeRecord bestNode, NavigationGraphEdge connectionEdge, int edgeIndex)
        {
            ...
            else
            {
                ...
                float gValue = bestNode.gValue + (childNode.LocalPosition - bestNode.node.LocalPosition).magnitude;
                float hValue = this.Heuristic.H(childNode, this.GoalNode);
                float fValue = F(gValue, hValue);

                if (childStatus == NodeStatus.Unvisited)
                {
                    this.NodeRecordArray.AddToOpen(UpdateChildNodeRecord(childNodeRecord, bestNode, gValue, hValue, edgeIndex));
                }
                else if (fValue < childNodeRecord.fValue || fValue == childNodeRecord.fValue && hValue < childNodeRecord.hValue)
                {
                    if (childStatus == NodeStatus.Closed)
                    {
                        this.NodeRecordArray.AddToOpen(UpdateChildNodeRecord(childNodeRecord, bestNode, gValue, hValue, edgeIndex));
                    }
                    else if (childStatus == NodeStatus.Open)
                    {
                        this.NodeRecordArray.Replace(childNodeRecord, UpdateChildNodeRecord(childNodeRecord, bestNode, gValue, hValue, edgeIndex));
                    }
                }
            }
        }

        protected NodeRecord UpdateChildNodeRecord(NodeRecord childNodeRecord, NodeRecord parent, float g, float h, int i)
        {
            childNodeRecord.parent = parent;
            childNodeRecord.gValue = g;
            childNodeRecord.hValue = h;
            childNodeRecord.fValue = F(g, h);
            childNodeRecord.StartNodeOutConnectionIndex = parent.node == this.StartNode ? i :  parent.StartNodeOutConnectionIndex;
